...
namespace Skyers
{
    ...
    bool Program::Create(std::string filename, ...)
    {
        ...
        if (!vertex_shader.empty())
        {
            ...
            auto vshader = g_resources.Get<Skyers::Shader>(vertex_shader, GL_VERTEX_SHADER);
            AddShader(vshader);
        }
        ...
        if (!fragment_shader.empty())
        {
            ...
            auto fshader = g_resources.Get<Skyers::Shader>(fragment_shader, (void*)GL_FRAGMENT_SHADER);
            AddShader(fshader);
        }
    ...
    {
        GLint uniform = GetUniform(name);
        if (uniform != -1) (uniform, 1, GL_FALSE, glm::value_ptr(value));
    }
